...
def soft_resample_ot(
    ...
    """
    Differentiable OT resampling via Sinkhorn algorithm.

    This same function works for:
      - standard OT resampling
      - neural-accelerated OT resampling

    To use neural acceleration, pass log_u_init and log_v_init.
    ...
    log_u_init, log_v_init : tf.Tensor or None
        Optional warm-start duals.
    normalize_cost : bool
        If True, rescales the cost matrix by its mean.
    clip_logK : float or None
        Optional clipping of log_K.
    ...
    particles_new : tf.Tensor, shape (N, d)
        Resampled particles after deterministic barycentric transport.
    ...
    # ------------------------------------------------------------------
    # Cost matrix between particles
    # ------------------------------------------------------------------
    C = pairwise_distance(particles, squared=True)
    
    # ------------------------------------------------------------------
    # Solve regularized OT problem
    # ------------------------------------------------------------------
    ...
    # move particles under the transport plan instead of discrete resampling
    # ------------------------------------------------------------------
    ...
    for n_iter in niter_grid:
        ...
        for eps in eps_grid:
            ...
            for rep in range(n_repeats):
                ...
                final_score = quality + lambda_speed * speed_penalty

                scores.append(float(final_score.numpy()))
                rmses.append(float(rmse.numpy()))
                esses.append(float(mean_ess.numpy()))
